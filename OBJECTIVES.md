...
| M3 | Serves a **named persona** | Two personas have a console surface and can be switched between in "Viewing as": transport manager operates it, facilities head gets its own KPI set, Breach-only priority actions, narrowed nav, plus the brief and the leadership export (R7). The line manager is **not** a console persona — it receives shift-sliced findings as a routed brief by rule (`audiences_for` → `LINE_MANAGER`), and nothing more. Its per-shift floor-readiness *screen* (was Task 8d) is reserve R9 — the statement's "who made it, who was late" is per-employee, `emp_legs` carries it, and the honest line is "next sprint" | Every brief is addressed to a named role and audience is assigned by rule; two personas have a surface, the line manager has a routed brief |
